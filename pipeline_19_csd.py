...
def run(subj_idx, ses_idx, epo_type, epo, json_file, spm_instance=None):
    ...
    out_fname = os.path.join(ses_out_path, f'csd_results_{epo_type}-epo.npz')
    ...
    if epo_type == 'visual':
        out_fname = os.path.join(ses_out_path, f'csd_results_{epo}_{epo_type}-epo.npz')
        data_file = os.path.join(ses_path,
    ...
    if os.path.exists(fname):# and not os.path.exists(out_fname):
        ...
        with spm_context(spm=spm_instance) as spm:
            ...
            for c_idx in range(len(cluster_vtx)):
                ...
                layer_ts, time, _ = load_source_time_series(base_fname, vertices=layer_verts, mu_matrix=MU)
    ...
    with spm_context() as spm:
        ...
        for epo_type,epo in zip(epo_types,epos):
            for subject_idx, session_idx in zip(subjects, sessions):
                run(subject_idx, session_idx, epo_type, epo, 'settings.json', spm_instance=spm)
