...
def run(subj_idx, ses_idx, epo_type, epo, condition, json_file):
    ...
    out_fname = os.path.join(ses_out_path, f'multilaminar_results_{epo_type}-epo_{condition}.npz')
    ...
    if len(epo):
        out_fname = os.path.join(ses_out_path, f'multilaminar_results_{epo}_{epo_type}-epo_{condition}.npz')
        data_file = os.path.join(
            ses_path,
